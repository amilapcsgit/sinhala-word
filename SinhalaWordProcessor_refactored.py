...
class SinhalaWordApp(QMainWindow):
    ...
    def save_file(self):
        ...
        if file_path:
            try:
                ...
                logger.info(f"Saved file: {os.path.basename(file_path)}")
    ...
    def clear_suggestion_area(self):
        ...
        # logger.debug("Suggestion area content cleared.")

    def accept_suggestion(self, singlish_word):
        """Accept a suggestion and replace the buffered text."""
        # logger.debug(f"accept_suggestion called with singlish_word: {singlish_word}")
        ...
        if not sinhala_word:
            ...
            # logger.debug(f"Fallback to phonetic for '{singlish_word}': '{sinhala_word}'")
        ...
        # logger.debug("Suggestion accepted and committed.")
    ...
    def commit_buffer(self):
        ...
        # logger.debug("Buffer committed successfully.")
    ...
    def on_text_changed(self):
        # logger.debug("on_text_changed called.")
    ...
    def suggestions(self, prefix: str, limit: int = 9):
        ...
        pref = prefix.lower()
        # logger.debug(f"Entering suggestions. Prefix: '{pref}'")
        if not pref:
            # logger.debug("Prefix is empty. Returning no suggestions.")
            return []
        ...
        try:
            ...
            logger.error(f"Error while fetching suggestions: {e}")

        # logger.debug(f"Suggestions generated: {results}")
        return results
